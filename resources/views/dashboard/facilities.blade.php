...
@section('title', 'Fasilitas & Fitur - DJM Admin')
...
@section('content')
    ...
    <div class="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
            ...
            <p class="mt-1 text-sm text-[#523828]">Kelola daftar fasilitas yang muncul sebagai pilihan pada form properti.</p>
        </div>
    </div>
    ...
    <div class="card mb-6">
        <div class="card-head">
            <div>
                <h3 class="text-base font-bold text-[#1F1611]">Tambah Fasilitas Baru</h3>
                <p class="mt-0.5 text-xs text-[#523828]">Fasilitas baru otomatis aktif dan langsung muncul di form properti.</p>
            </div>
        </div>
        <form action="{{ route('facilities.store') }}" method="POST"
            ...
            <input type="text" name="name" value="{{ old('name') }}" required maxlength="100"
                placeholder="Contoh: Smart Home, Jacuzzi, Lift Pribadi..."
    ...
    <div class="card overflow-hidden">
        <div class="card-head">
            <div>
                <h3 class="text-base font-bold text-[#1F1611]">Daftar Fasilitas ({{ count($facilities) }})</h3>
                <p class="mt-0.5 text-xs text-[#523828]">Fasilitas yang masih dipakai properti tidak dapat dihapus.</p>
            </div>
        </div>

        {{-- Filter Bar --}}
        <div class="border-b border-[#F0E6D2] bg-[#FBF6EC]/40 px-6 py-4">
            <form method="GET" action="{{ route('dashboard.facilities') }}" class="flex flex-col gap-4 sm:flex-row sm:items-end">
                <div class="grid flex-1 grid-cols-1 gap-3 sm:grid-cols-2">
                    {{-- Pencarian Kata Kunci --}}
                    <div>
                        <label for="search" class="mb-1 block text-xs font-bold uppercase tracking-wider text-[#946E4B]">Cari Fasilitas</label>
                        <div class="relative">
                ...
                <div class="flex gap-2">
                    ...
                    @if(request()->anyFilled(['search', 'status']))
                        <a href="{{ route('dashboard.facilities') }}" class="rounded-lg border border-[#EADFCB] bg-white px-4 py-2 text-center text-xs font-medium text-[#523828] transition hover:bg-[#F9F0D6]">
        ...
        <div class="hidden overflow-x-auto lg:block">
            <table class="w-full text-left text-sm">
                <thead class="border-b border-[#F0E6D2] bg-[#FBF6EC] text-xs uppercase tracking-wider text-[#946E4B]">
                    <tr>
                        <th class="px-4 py-4 xl:px-6 font-bold">Nama Fasilitas</th>
                        <th class="px-4 py-4 xl:px-6 font-bold">Dipakai Properti</th>
                ...
                <tbody class="divide-y divide-[#F0E6D2]">
                    ...
                    <tr class="transition hover:bg-[#FBF6EC]/60">
                        <td class="px-4 py-4 xl:px-6 font-semibold text-[#1F1611]">{{ $facility->name }}</td>
                        ...
                        <td class="px-4 py-4 xl:px-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <button type="button"
                                    ...
                                    data-action="{{ route('facilities.update', $facility->id) }}"
                                ...
                                @if($facility->usage_count > 0)
                                    <button type="button" disabled
                                        class="cursor-not-allowed rounded-lg border border-[#EADFCB] bg-[#F3EFE6] px-3 py-1.5 text-xs font-medium text-[#B0A48F]"
                                        title="Tidak dapat dihapus karena masih dipakai {{ $facility->usage_count }} properti">
                                        <i class="fa-solid fa-trash"></i> Hapus
                                    </button>
                                @else
                                    <form action="{{ route('facilities.destroy', $facility->id) }}" method="POST"
                                        onsubmit="return confirm('Apakah kamu yakin ingin menghapus fasilitas {{ addslashes($facility->name) }}?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-14 text-center">
                            ...
                            <p class="text-sm font-semibold text-[#1F1611]">Belum ada fasilitas</p>
                            <p class="mt-1 text-xs text-[#523828]">Tambahkan fasilitas pertama melalui form di atas.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
            ...
            <div class="p-4">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0 flex-1">
                        <h4 class="text-sm font-semibold text-[#1F1611]">{{ $facility->name }}</h4>
                        <p class="mt-1.5 text-xs text-[#523828]">
                            <i class="fa-solid fa-building text-[#946E4B]"></i> {{ $facility->usage_count }} properti
                        </p>
                        <span class="mt-2 inline-block rounded-full px-2.5 py-1 text-xs font-semibold {{ $facility->is_active ? 'bg-[#F9F0D6] text-[#523828]' : 'bg-[#F3EFE6] text-[#8A7B63]' }}">
                ...
                <div class="mt-3 flex flex-wrap gap-2">
                    ...
                    @else
                        <form action="{{ route('facilities.destroy', $facility->id) }}" method="POST"
                            onsubmit="return confirm('Apakah kamu yakin ingin menghapus fasilitas ini?')" class="min-w-[120px] flex-1">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
    ...
    <dialog id="edit-dialog" class="w-[calc(100%-2rem)] max-w-md rounded-2xl border border-[#EADFCB] bg-white p-0 shadow-2xl backdrop:bg-black/40">
        <form method="POST" id="edit-form" class="max-h-[85vh] overflow-y-auto p-5 sm:p-6">
            ...
            <h3 class="text-lg font-bold text-[#1F1611]">
                <i class="fa-solid fa-pen-to-square mr-2 text-[#946E4B]"></i>Edit Fasilitas
            </h3>
            <p class="mt-0.5 mb-4 text-xs text-[#523828]">Mengubah nama akan berlaku untuk pilihan form properti ke depan.</p>

            <label for="edit-name" class="mb-1 block text-xs font-bold uppercase tracking-wider text-[#946E4B]">Nama Fasilitas</label>
            ...
            <label class="mt-3 flex cursor-pointer items-center gap-2.5 rounded-lg border border-[#EADFCB] bg-[#FBF6EC] px-3 py-2.5 text-sm font-semibold text-[#1F1611]">
                ...
                Fasilitas aktif (tampil di form properti)
            </label>
